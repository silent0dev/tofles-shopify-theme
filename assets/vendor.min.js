console.log('Wokiee Shopify Theme. Version 2.0.4. https://themeforest.net/item/wokiee-multipurpose-shopify-theme/22559417')
...
function swatchVariantHandler(_parent, variant) {
  ...
  var oldVariantId = currentURL.searchParams.get("variant");
  var newVariantId = variant.id;
  var variantUrl = "";

  currentURL.searchParams.set("variant", newVariantId);
  
  console.log("old", oldVariantId, "new", newVariantId);
  console.log("currentURL", currentURL);

  history.pushState(history.state, document.title, currentURL.href);
  
  if (typeof DoublyGlobalCurrency !== typeof undefined) {
    console.log("1. defined");
    var from_currency = "USD";
    var to_currency = DoublyGlobalCurrency.cookie.read()

    var price_cents = variant.price;
    var compare_price_cents = variant.compare_at_price;
    var price_dollars = (variant.price / 100.0).toFixed(3);
    var compare_price_dollars = (variant.compare_at_price / 100.0).toFixed(3);

    var converted_price = DoublyGlobalCurrency.convert(price_dollars, from_currency, to_currency);
    var converted_compare_price = DoublyGlobalCurrency.convert(compare_price_dollars, from_currency, to_currency);

    var currency_format = `<span class="money" doubly-currency="${to_currency}" doubly-currency-usd="${price_cents}" doubly-currency-${to_currency.toLowerCase()}="${converted_price}">${DoublyGlobalCurrency.moneyFormats[to_currency][DoublyGlobalCurrency.format]}</span>`;
    var currency_compare_format = `<span class="money" doubly-currency="${to_currency}" doubly-currency-usd="${compare_price_cents}" doubly-currency-${to_currency.toLowerCase()}="${converted_compare_price}">${DoublyGlobalCurrency.moneyFormats[to_currency][DoublyGlobalCurrency.format]}</span>`;

    var price_str = DoublyGlobalCurrency.formatMoney(converted_price * 100, currency_format, (to_currency === "USD"), false);
    var price_comare_str = DoublyGlobalCurrency.formatMoney(converted_compare_price * 100, currency_compare_format, (to_currency === "USD"), false);
  } else {
